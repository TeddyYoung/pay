...
<html lang="en">
	...
	<body>
		<form action="<%=basePath%>companyDefaultRate/update.do" name="updateForm" id="updateForm" method="post">
			<input type="hidden" name="id" value="${companyDefaultRate.id}"/>
			<div id="zhongxin">
				<table id="table_report" class="table table-striped table-bordered table-hover">
					<tr>
						<td style="width:70px;text-align: right;padding-top: 13px;">公司类型:</td>
						<td><input type="text" name="companyType" value="${companyDefaultRate.companyType}"/></td>
						<td style="width:70px;text-align: right;padding-top: 13px;">公司类型名称:</td>
						<td><input type="text" name="companyTypeName" value="${companyDefaultRate.companyTypeName}"/></td>
					</tr>
					<tr>
							...
							<select class="chzn-select" name="channelId" id="channelId" data-placeholder="请选择">
								...
								<c:if test="${channelList != null}">
									<c:forEach items="${channelList}" var="var">
										<option value="${var.id}" <c:if test="${companyDefaultRate.channelId == var.id}">selected</c:if> >${var.channelName}</option>
									</c:forEach>
								</c:if>
							</select>
						</td>
						<td style="width:120px;text-align: right;padding-top: 13px;">交易费率:</td>
						<td><input type="text" name="tradeRate" value="${companyDefaultRate.tradeRate}"/></td>
					</tr>
					<tr>
						<td style="width:120px;text-align: right;padding-top: 13px;">正常结算费用:</td>
						<td><input type="text" name="normalFee" value="${companyDefaultRate.normalFee}"/></td>
						<td style="width:120px;text-align: right;padding-top: 13px;">晚间结算费用:</td>
						<td><input type="text" name="nightFee" value="${companyDefaultRate.nightFee}"/></td>
					</tr>
					<tr>
						<td style="width:120px;text-align: right;padding-top: 13px;">节假日结算费用:</td>
						<td><input type="text" name="holidayFee" value="${companyDefaultRate.holidayFee}"/></td>
						<td style="width:120px;text-align: right;padding-top: 13px;">结算上限:</td>
						<td><input type="text" name="cashMax" value="${companyDefaultRate.cashMax}"/></td>
					</tr>
					<tr>
						<td style="width:120px;text-align: right;padding-top: 13px;">品牌id:</td>
						<td><input type="text" name="brandId" value="${companyDefaultRate.brandId}"/></td>
						<td></td>
						<td></td>
					</tr>
					<tr>
